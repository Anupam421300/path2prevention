...
function showRiskBreakdown(riskScore) {
  ...
  container.innerHTML = `
    ...
    <div style="height:8px;background:var(--border-glass);border-radius:8px;overflow:hidden;margin-bottom:26px;">
      <div style="height:100%;background:${color};width:${internalScore}%;border-radius:8px;transition:width 0.8s ease;"></div>
    </div>
    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:22px;">
      ${breakdown.map(b => `
        <div style="padding:16px;border-radius:14px;background:var(--bg-surface-low);display:flex;gap:14px;align-items:flex-start;">
          ...
          <div style="flex:1;">
            ...
            <p style="font-size: 16px;color:var(--text-secondary);line-height:1.5;">${b.note}</p>
          </div>
        </div>
      `).join('')}
    </div>
    <div style="padding:16px;background:#006c49/5%;border-radius:14px;display:flex;gap:10px;border:1px solid rgba(0,108,73,0.1);background:rgba(0,108,73,0.04);">
      ...
      <p style="font-size: 16px;color:var(--text-secondary);line-height:1.5;">Based on the <strong>National Diabetes Prevention Program (DPP)</strong>. Small lifestyle changes can reduce risk by up to 58%.</p>
    </div>
  `;
...
async function showEvidencePanel(evidenceRefs) {
  ...
  try {
    ...
    content.innerHTML = sources.map(s => `
      <div style="padding:20px;background:var(--bg-surface-low);border-radius:16px;margin-bottom:14px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
          <div style="padding:2px 8px;background:var(--text-primary);color:var(--text-primary);border-radius:4px;font-size: 14px;font-weight:700;">${s.publisher || 'Study'}</div>
          <span style="font-size: 14px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--text-muted);">${s.publisher || ''}</span>
        </div>
        <h4 style="font-size: 18px;font-weight:700;margin-bottom:10px;color:var(--text-primary);line-height:1.4;">${s.title}</h4>
        <p style="font-size: 16px;color:var(--text-secondary);line-height:1.5;margin-bottom:12px;">${s.snippet || ''}</p>
...
function defaultEvidenceSources() {
  ...
  return defaults.map(d => `
    <div style="padding:20px;background:var(--bg-surface-low);border-radius:16px;margin-bottom:14px;">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
        <div style="padding:2px 8px;background:${d.color};color:var(--text-primary);border-radius:4px;font-size: 14px;font-weight:700;">${d.pub}</div>
      </div>
      <h4 style="font-size: 18px;font-weight:700;margin-bottom:10px;color:var(--text-primary);line-height:1.4;">${d.title}</h4>
      <p style="font-size: 16px;color:var(--text-secondary);line-height:1.5;">${d.snippet}</p>
    </div>
  `).join('');
}
